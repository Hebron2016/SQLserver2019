...
SELECT id,ano,
	...
	titulo
	ELSE
	(SUBSTRING(titulo,1,10)+'...') END  FROM Filme
WHERE id in
(
...
SELECT num,data_Fabricacao,DATEDIFF(MONTH,data_Fabricacao,GETDATE()) AS qtdMeses FROM DVD
WHERE filmeID IN
(          
	SELECT ID FROM Filme
	WHERE titulo LIKE 'inter%'
	)

SELECT dvdNUM, data_locacao,data_devolucao,DATEDIFF(DAY,data_locacao,data_devolucao) AS diasAlugados, valor  FROM Locacao
...
SELECT nome, (logradouro +''+ num) AS endCompleto, SUBSTRING(cep,1,5)+'-'+SUBSTRING(cep,6,3) as Dividido FROM Cliente
WHERE num_cadastro IN
(
	SELECT num_cadastro FROM Locacao
	WHERE dvdNUM IN
	(
		SELECT num FROM DVD
		WHERE num = 10002
