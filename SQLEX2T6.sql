...
SELECT cli.num_cadastro, cli.nome, fil.titulo,midia.data_Fabricacao, alo.valor
...
SELECT cli.num_cadastro, cli.nome, alo.data_locacao, COUNT(alo.dvdNUM) AS qtdDVD 
...
SELECT cli.num_cadastro, cli.nome, alo.data_locacao, SUM(alo.valor) AS valorTotal
FROM Cliente cli, Locacao alo
WHERE cli.num_cadastro = alo.clientenum_cadastro
AND alo.valor IN
(
...
SELECT cli.num_cadastro, cli.nome, CASE WHEN (COUNT(alo.data_locacao) >= 2) THEN data_locacao END AS valorTotal
FROM Cliente cli, Locacao alo
WHERE cli.num_cadastro = alo.clientenum_cadastro
